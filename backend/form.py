...
def export_xlsx(
    ...
    for r in rows:
        submitted_val = r.submitted_at
        if getattr(submitted_val, "tzinfo", None) is not None:
            try:
                submitted_val = submitted_val.replace(tzinfo=None)
            except Exception:
                submitted_val = str(submitted_val)
        ws_a.append([r.qid, r.qtext, r.answer_value, (r.user_email or "anonimo"), submitted_val])
